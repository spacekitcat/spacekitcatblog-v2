...
const renderSnakes = (snakeState) => {
  let i = 0;
  let statclass = 'spacerace__environment-stats';
  if (!stateVisibilityCounterRegister) {
    statclass += ' spacerace__environment-stats--alt';
  }
  stateVisibilityCounterRegister = !stateVisibilityCounterRegister;

  const snakes = snakeState.map(state => (
    <div className="spacerace__container" key={i}>
      <div className={`${getPositionClass(state.position)} spacerace__sprite--number${i}`} key={i++} />
    </div>
  ));
  return (
    <div className="spacerace__environment-wrapper">
      <div className="spacerace__environment-wrongscreensize">
      ...
      <div className="spacerace__environment-container">
        <div className={statclass}>
          {snakeState[0].position
          && `${sortBy(snakeState, o => o.position.x)[snakeState.length - 1].number} IS WINNING THE SPACE RACE`
        }
        </div>
        <div className="spacerace__environmentstartlineleft">START</div>
        <div className="spacerace__environmentfinishline">FINISH</div>
        {snakes}
        <div className="spacerace__environmentstartlineright">START</div>
        <div className="spacerace__environmentfinishlinebottom">FINISH</div>
      </div>
    </div>
  );
};
